...
class ActionGutter(Gtk.Widget):
    ...
    def activate(self, chunk: DiffChunk, x: float, y: float):
        if chunk[0] == 'conflict' and getattr(self, 'filediff', None) is not None:
            ...
            conflict_menu.set_pointing_to(rect)
            conflict_menu.popup()
            return
    ...
    def _make_conflict_menu(self, chunk):
        ...
        keep_left = Gtk.Button(label=_("Keep Left (Accept Left)"))
        keep_right = Gtk.Button(label=_("Keep Right (Accept Right)"))
        keep_both_l = Gtk.Button(label=_("Keep Both (Left then Right)"))
        keep_both_r = Gtk.Button(label=_("Keep Both (Right then Left)"))

        box.append(keep_left)
        box.append(keep_right)
        box.append(keep_both_l)
        box.append(keep_both_r)
        ...
        keep_left.connect('clicked', on_keep_left)
        keep_right.connect('clicked', on_keep_right)
        keep_both_l.connect('clicked', on_keep_both_l)
        keep_both_r.connect('clicked', on_keep_both_r)
